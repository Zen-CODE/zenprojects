...
class PromptComposer:
    """Class managing prompt composition and data fetching."""

    @staticmethod
    def _has_auth(user_auth: AUTH_DICT, required_auth: AUTH_DICT) -> Tuple[bool, str]:
        """Return (True, "") if the user has all on the required authentication, (False, "Reason") otherwise"""                
        ...
        for required_key, required_values in required_auth.items():
            ...
            if any(user_value in required_values for user_value in user_values):
                return (True, "")
        return (False, f"Missing {required_auth}")

    @staticmethod
    def compose(raw_prompt_text: str, user_auth: dict[str, list[str]], fetches: dict[callable]) -> str:
        ...
        for tag in soup.find_all(True):
            if tag.name in fetch_keys:
                # We have a tag for which a data fetch is defined. Does it require auth?
                auth_attr = tag.get("auth")
                authorised = not bool(auth_attr)
                if not authorised:
                    required_auth: AUTH_DICT = ast.literal_eval(auth_attr)
                    authorised, reason = PromptComposer._has_auth(user_auth, required_auth)
                                                            
                    del tag["auth"]
                    if authorised:
                        # TODO: Collects the fetches and run async in parallel
                        func = fetches[tag.name]
...
Member list
------------
<get_member_list auth="{'roles': ['NONSENSE'], 'consents': ['VIEW_POLICY']}">
OR - User has a no-existent role or view policy access
</get_member_list>
"""

# ============== Map of tag names to data fetch functions
fetches = {
...
print("\n===============================================  1. Spouse prompt")
print(PromptComposer.compose(markdown_template, user_auth={"roles": ["SP"], "consents": []}, fetches=fetches))

print("\n#================================================ 2. Principle pass")
print(PromptComposer.compose(markdown_template, user_auth={"roles": ["PH", "PP"], "consents": ["VIEW_POLICY"]}, fetches=fetches))
